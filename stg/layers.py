...
class FeatureSelector(nn.Module):
    def __init__(self, input_dim, sigma, device):
        ...
        self.noise = torch.randn(self.mu.size()) #TODO Change behavior in eval
        #self.mu = 0.01*torch.randn(input_dim, requires_grad=True)
        self.sigma = sigma
        self.device = device
    
    def forward(self, prev_x):
        #z = self.mu + self.sigma*Variable(torch.randn(self.mu.size()))*self.training #self.isTraining
        z = self.mu + self.sigma*self.noise.normal_()*self.training #self.isTraining
    ...
    def hard_sigmoid(self, x):
        ...
        #return torch.max(torch.tensor(0,dtype=torch.float), torch.min(torch.tensor(1,dtype=torch.float), torch.tensor(0.5,dtype=torch.float)+x))
