...
class STGModel(MLPModel):
    def __init__(self, input_dim, output_dim, hidden_dims, batch_norm=None, dropout=None, activation='relu',
        ...
        # self.reg =  torch.sum(self.regularizer((self.FeatureSelector.mu + 0.5)/self.FeatureSelector.sigma))

    def forward(self, x):
        return self.FeatureSelector(x)
...
class STGClassificationModel(MLPModel, ModelIOKeysMixin):
    ...
    def forward(self, feed_dict):
        ...
        if self.training:
            ...
            reg = torch.mean(self.reg((self.mu + 0.5)/self.sigma)) 
            total_loss = loss + self.lam * reg 
            return total_loss, dict(), dict() #dict(), dict()
        else:
...
class MLPCoxModel(MLPModel, ModelIOKeysMixin):
    def __init__(self, input_dim, nr_classes, hidden_dims, batch_norm=None, dropout=None, activation='relu'):
        super().__init__(input_dim, nr_classes, hidden_dims,
                         batch_norm=batch_norm, dropout=dropout, activation=activation)
        self.loss = PartialLogLikelihood #nn.CrossEntropyLoss()
        self.noties = 'noties'
...
'''
class STGClassificationModel(STGModel, ModelIOKeysMixin):
    def __init__(self, input_dim, nr_classes, hidden_dims, batch_norm=None, dropout=None, activation='relu',
                 sigma=1.0):
        super().__init__(input_dim, nr_classes, hidden_dims,
                         batch_norm=batch_norm, dropout=dropout, activation=activation, sigma=sigma)
        #self.FeatureSelector = FeatureSelector(input_dim, sigma)
        self.softmax = nn.Softmax()
        self.loss = nn.CrossEntropyLoss()
        #self.reg =  torch.sum(self.regularizer((self.FeatureSelector.mu + 0.5)/self.FeatureSelector.sigma))
    ...
    def forward(self, feed_dict):
        ...
        if self.training:
            loss = self.loss(logits, self._get_label(feed_dict))
            reg = torch.sum(self.regularizer((self.FeatureSelector.mu + 0.5)/self.FeatureSelector.sigma)) 
            total_loss = loss + reg 
            return total_loss, dict(), dict()
        else:
...
if __name__=='__main__':
    #stg_layer = FeatureSelector(10, 0.1)
    stg_model = STGRegressionModel(10, 1, [5, 5], sigma=0.1)
    #print('stg_layer.training : {}'.format(stg_layer.training))
    #stg_model.eval()
    print(stg_model.training)
    print(stg_model.FeatureSelector.training)
    for param in stg_model.FeatureSelector.parameters(): print(torch.sum(param))
    #print(torch.sum(stg_model.FeatureSelector.parameters()))
    #print(torch.sum(stg_model.FeatureSelector.mu))
    print("ok")
    print(stg_model.mu)
